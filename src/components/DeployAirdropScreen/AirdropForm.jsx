...
class AirdropForm extends Component {
  ...
  _renderDropdownList (tokensArray) {
    const Caret = () => (<i className='fa fa-caret-down' style={{ display: 'inline', color: 'black', float: 'right', marginRight: 15, fontSize: 25 }} />)
              ...
              <div>
                {
                  tokensArray.map(token => (
                    <div key={token.contract.address} onClick={(() => this._selectToken(token))} className='token'>{token.contract.symbol}&mdash;{token.contract.address}{token.isEther ? <Caret /> : null}</div>
                  ))
                }
